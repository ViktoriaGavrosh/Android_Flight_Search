...
interface FlightDao {
    ...
    @Query(
        "SELECT * FROM airport WHERE iata_code LIKE '%' || :code || '%' " +
                "OR name LIKE '%' || :code || '%'"
    )
    ...
    @Query("DELETE FROM favorite WHERE departure_code = :departureCode AND " +
            "destination_code = :arrivalCode")
    ...
    @Query(
        "SELECT f.id, departure_code, a.name AS 'departure_name', destination_code, b.name AS " +
    ...
    @Query(
        "SELECT * FROM favorite WHERE departure_code = :departureCode AND " +
